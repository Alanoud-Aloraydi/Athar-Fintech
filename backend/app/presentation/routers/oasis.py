...
Exposes the optimized Oasis-state endpoint and the transaction-preview
("simulate") endpoint consumed by the Spline 3D frontend. Like the other
routers, this depends *only* on `OasisFacade` — it never imports
`OasisRepository`, `GamificationEngine`, or `CategorizationEngine`
...
async def get_oasis_state(
    ...
    (weather_condition, visual_aura, streak_multiplier, mood_message) and
    the `visible_palm_count` derived from them for the Spline scene.
...
@router.post(
    "/{user_id}/simulate",
    response_model=OasisSimulationResponseDTO,
    summary="Preview a hypothetical transaction's effect on the Oasis, without persisting anything",
)
async def simulate_transaction_impact(
    user_id: UUID,
    payload: OasisSimulationRequestDTO,
    facade: OasisFacade = Depends(get_oasis_facade),
    current_user_id: str = Depends(get_current_user_id),
) -> OasisSimulationResponseDTO:
    """
    Runs a hypothetical transaction through the same Categorization +
    Gamification pipeline a real `POST /transactions/` call would use,
    and returns the predicted category, Oasis impact, and before/after
    palm count — without writing anything to any table.

    Powers the Farm screen's "try a transaction" preview: the Flutter
    client can call this as the user types a hypothetical amount and
    description, animate the projected palm count, and only then decide
    whether to actually log it via `POST /transactions/`.
    """
    require_matching_user(str(user_id), current_user_id)
    try:
        return await run_in_threadpool(
            facade.simulate_transaction_impact, str(user_id), payload
        )
    except PersistenceError as exc:
        logger.exception("Failed to simulate Oasis impact for user_id=%s", user_id)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=_UPSTREAM_ERROR_DETAIL,
        ) from exc
